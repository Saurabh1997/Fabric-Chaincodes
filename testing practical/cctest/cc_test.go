...
func TestCalculator(t *testing.T) {
	...
	ccArgs = SetupArgsArray("invoke", "multiply", "2")
	response = stub.MockInvoke("TxNultiply", ccArgs)
	result, _ = strconv.ParseInt(string(response.Payload), 10, 64)
	t.Logf("Multiply Received Result = %d", result)
...
func SetupArgsArray(funcName string, args ...string) [][]byte {
	...
	//make is inbuilt function accepting (data type, length of array to make )
